	...
	if(document.adbranch.description.value == "")
	{
		...
		return false;
          ...
          <form id="adbranch" name="adbranch" method="post" action="" onsubmit="return validation()">
            ...
            <table width="689" height="44" border="1">
              <tr>
                <td width="229"><strong>Product information</strong></td>
                <td width="142"><strong>Image</strong></td>
                <td width="102"><strong>Color</strong></td>
                <td width="135" align="center"><strong>Quantities sold</strong></td>
                <td width="143" align="center"><strong>&nbsp;Amount collected</strong></td>                
              </tr>
              <?php
				...
				while($row1 = mysqli_fetch_array($resultbranch1))
				{
					...
					{
						 $reccounts = 0;
					 $resultprodrec = mysqli_query($con, "SELECT * FROM  products where prod_id='$row1[prod_id]' AND branch_id='$_POST[group]'");
					 $rowprod = mysqli_fetch_array($resultprodrec);
						  if(mysqli_num_rows($resultprodrec) == 1)
						 {
						 $reccounts = 1;
						 }
					}
