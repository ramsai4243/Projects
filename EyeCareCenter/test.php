...
if(isset($_POST['token']) && $_POST['token'] == $_SESSION['token'])
{
	...
	//$insi = mysqli_insert_id();
	$upd = "UPDATE appointment SET status='Done' where app_id='$_GET[appid]'";
    $res2=mysqli_query($con, $upd);
	 
 	if($res2)
  	{
 		$rcsucc = "<font  color='#009900' >Test Details saved successfully.";
 	}
...
if(isset($_POST['submit']))
{
	...
	$sq ="select max(test_id) from test";
	$maxrec = mysqli_query($con,$sq);
	$maxarr = mysqli_fetch_array($maxrec);
	$insi=$maxarr[0];
	//echo $insi;

	?>
    <a href="prescription.php?testi=<?php echo $insi; ?>">Add a Prescription </a>
    <?php
}
else
{
...
$rowsapprec2 = mysqli_fetch_array($resultq2);
				...
				echo "</td>
           		...
           		while($row = mysqli_fetch_array($resultq1))
  				{
  					echo "<tr>";
